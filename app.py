...
# --- BLOQUE ANEXO: ZONA DE PUBLICIDAD PRIVADA ---
...
# [AQUÍ EMPIEZA TU CÓDIGO ORIGINAL SIN ALTERAR NADA]
...
# --- BLOQUE ANEXO: VISUALIZACIÓN DEL AVISO ---
if st.session_state.anuncio_vip:
    st.markdown(f'<div style="background:#3b82f6; color:white; padding:15px; border-radius:16px; text-align:center; margin-bottom:20px;">🌟 <b>PUBLICIDAD:</b> {st.session_state.anuncio_vip}</div>', unsafe_allow_html=True)
# ---------------------------------------------
...
# --- TU SISTEMA INTELIGENTE DE EXTRACCIÓN (TOTALMENTE ORIGINAL) ---
...
def escanear_agenda_real(ciudad, categoria):
    ...
    if not eventos_encontrados:
        contingencia = {
            "gratis": [{"titulo": f"Feria Artesanal y Mercado Cultural Local", "descripcion": f"Muestras de arte en vivo y stands en {ciudad}.", "link": "https://www.google.com/search?q=agenda+cultural+gratis+" + ciudad}],
            "rumba": [{"titulo": f"Agenda de Conciertos y Clubes Locales", "descripcion": f"Eventos reportados en las zonas de entretenimiento de {ciudad}.", "link": "https://www.google.com/search?q=mejores+discotecas+rumba+hoy+" + ciudad}],
            "cc": [{"titulo": f"Exposiciones Especiales en Centros Comerciales", "descripcion": f"Muestras comerciales de temporada en {ciudad}.", "link": "https://www.google.com/search?q=eventos+centros+comerciales+" + ciudad}],
            "cine": [{"titulo": f"Funciones de Estrenos de Películas", "descripcion": f"Programación en los multiplex de {ciudad}.", "link": "https://www.google.com/search?q=cartelera+cine+estrenos+" + ciudad}],
            "deportes": [{"titulo": f"Encuentros de Ocio Activo", "descripcion": f"Caminatas ecológicas en los alrededores de {ciudad}.", "link": "https://www.google.com/search?q=deportes+actividades+aire+libre+" + ciudad}]
        }
...
def renderizar_pestaña(tab_objeto, clave_cat, texto_header, badge_type, icono):
    with tab_objeto:
        ...
        for ev in lista_eventos:
            ...
            with col_btn1: st.link_button("🌐 Ver Detalles", ev['link'], use_container_width=True)
            with col_btn2: st.link_button("📲 Mandar al Grupo", f"https://api.whatsapp.com/send?text={urllib.parse.quote(ev['titulo'])}", use_container_width=True)
...
st.markdown("---")
st.caption("⚡ Portal Comercial v8.0 | Motor de Extracción Híbrido en Tiempo Real.")
